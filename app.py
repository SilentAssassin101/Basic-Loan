...
def get_loan_obj(id):
    ...
    return Loan(loanDict['id'], loanDict['memo'], loanDict['amount'], loanDict['term'], loanDict['remaining'], True)
...
class LoanAppGUI(tk.CTk):
    def __init__(self):
        ...
        mainLabel = tk.CTkLabel(master=frame, text='Loan App', font=('Roboto', 24))
        mainLabel.pack(pady=12, padx=10)
        
        self.loanListbox = otk.Listbox(master=frame, font=('Roboto', 12), bg='#3c3c3c', width=100, height=10, bd=0, highlightthickness=0, fg='white')
        self.loanListbox.pack(pady=12, padx=10)
        
        self.loanInfoFrame = tk.CTkFrame(master=frame)
        self.loanInfoFrame.pack(pady=12, padx=10)
        
        self.loanInfoLabel = tk.CTkLabel(master=self.loanInfoFrame, text='Loan Info', font=('Roboto', 16))
        self.loanInfoLabel.pack(pady=12, padx=10)
        
        self.loanInfoText = tk.CTkTextbox(master=self.loanInfoFrame, font=('Roboto', 16), width=250)
        self.loanInfoText.pack(pady=12, padx=10)
        
        self.makePaymentButton = tk.CTkButton(master=self.loanInfoFrame, text='Make Payment', font=('Roboto', 16), command=self.makePayment)
        self.makePaymentButton.pack(pady=12, padx=10)
        
        self.addLoanButton = tk.CTkButton(master=frame, text='Add Loan', font=('Roboto', 16), command=self.openAddLoanWindow)
        self.addLoanButton.pack(pady=12, padx=10)
        
        self.exitButton = tk.CTkButton(master=frame, text='Exit', font=('Roboto', 16), hover_color='red', command=root.quit)
    ...
    def show_loan_info(self, event):
        ...
        self.loanInfoText.insert('end', f"Monthly Payment: ${loan['monthlyPayment']:,.2f}\n")
        self.loanInfoText.insert('end', f"Remaining: ${loan['remaining']:,.2f}")
    ...
    def addLoan(self, id, memo):
        """This method is meant to load an already created loan to the UI since there is no way to handle state in the UI
    ...
    def openAddLoanWindow(self):
        addLoanWindow = LoanAppAddLoanGUI(self)
        
    def makePayment(self):
        dialog = tk.CTkInputDialog(text='Enter payment amount', title='Make Payment', font=('Roboto', 16))
...
class LoanAppAddLoanGUI(tk.CTkToplevel):
    def __init__(self, mainAppUI):
        ...
        mainLabel = tk.CTkLabel(master=frame, text='Add Loan', font=('Roboto', 24))
        mainLabel.pack(pady=12, padx=10)
        
        memoLabel = tk.CTkLabel(master=frame, text='Memo:', font=('Roboto', 16))
        memoLabel.pack(pady=12, padx=10)
        
        self.memoEntry = tk.CTkEntry(master=frame, font=('Roboto', 16))
        self.memoEntry.pack(pady=12, padx=10)
        
        amountLabel = tk.CTkLabel(master=frame, text='Amount:', font=('Roboto', 16))
        ...
        termLabel = tk.CTkLabel(master=frame, text='Term:', font=('Roboto', 16))
        termLabel.pack(pady=12, padx=10)
        
        self.termEntry = tk.CTkEntry(master=frame, font=('Roboto', 16))
        self.termEntry.pack(pady=12, padx=10)
        
        saveButton = tk.CTkButton(master=frame, text='Save', font=('Roboto', 16), command=self.save_loan)
        saveButton.pack(pady=12, padx=10)
        
        cancelButton = tk.CTkButton(master=frame, text='Cancel', font=('Roboto', 16), command=self.destroy)
